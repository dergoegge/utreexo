...
func verifyBatchProof(bp BatchProof, roots []Hash, numLeaves uint64,
	...
	trees := make([]miniTree, len(computablePositions))
	treeIndex := len(trees) - 1
	...
	for len(targetNodes) > 0 {
		...
		trees[treeIndex] = miniTree{parent: node{Val: hash, Pos: parentPos}, left: left, right: right}
		treeIndex--

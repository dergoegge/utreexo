...
// ProbeCache returns the needed positions (inlcuding the targets themselves) to proof the `targets`,
// the cached positions and their hashes.
func (p *Pollard) ProbeCache(targets []uint64) (needed []uint64, cached []uint64, hashes []Hash) {
	targetsAndProof := mergeSortedSlices(ProofPositions(targets, p.numLeaves, p.rows()), targets)
	for _, proofPos := range targetsAndProof {
		node, _, _, err := p.readPos(proofPos)
		if err != nil {
			panic(err)
		}

		if node == nil || node.data == empty {
			needed = append(needed, proofPos)
			continue
		}

		cached = append(cached, proofPos)
...
func (p *Pollard) addOne(add Hash, remember bool) error {
	...
	for ; (p.numLeaves>>h)&1 == 1; h++ {
		...
		} else if h == 0 && !remember && leftRoot.niece[0] == leftRoot {
			// sibiling is memorable also remember this node
			n.niece[0] = n
		}
